...
//        try {
//            customerInfoResponse = customerDubboService.getCustomerInfoByCifCode(cifCode);
//        } catch (Exception e) {
...
//        CustomerInfoRp customerInfo;
//        if (customerInfoResponse.getStatusCode().is2xxSuccessful()) {
//            customerInfo = Optional.ofNullable(customerInfoResponse.getBody())
//                    .orElseThrow(() -> {
//                        log.info(MessageData.MESSAGE_LOG, transactionId, MessageData.CUSTOMER_ACCOUNT_NOT_FOUND.getMessageLog());
//                        return new DataNotFoundException(MessageData.CUSTOMER_ACCOUNT_NOT_FOUND.getKeyMessage(), MessageData.CUSTOMER_ACCOUNT_NOT_FOUND.getCode());
//                    });
//
//            if (!customerInfo.getCustomerStatus().equalsIgnoreCase("ACTIVE")) {
...
//            if (customerInfoResponse.getStatusCode().equals(HttpStatus.NOT_FOUND)) {
//                log.info(MessageData.MESSAGE_LOG, transactionId, MessageData.CUSTOMER_ACCOUNT_NOT_FOUND.getMessageLog());
...
//            } else {
//                log.info(MessageData.MESSAGE_LOG, transactionId, customerInfoResponse.getBody());
//                throw new ServerErrorException();
//            }
//        }

        // Check if the banking account is active
//        Boolean isBankingAccountActive = accountDubboService.isBankingAccountIsActive(customerInfo.getCustomerId());
//        if (!isBankingAccountActive) {
...
//        financialInfo.setCustomerId(customerInfo.getCustomerId());
        financialInfo.setCustomerId("123456789");
    ...
    @Override
    public DataResponseWrapper<Object> getAllInfoIsByStatus(Integer pageNumber, Integer pageSize, String status, String transactionId) {
        ...
        List<FinancialInfoRp> financialInfoRpList = financialInfoPage.getContent().stream().map(financialInfo -> convertToFinancialInfoRp(financialInfo, false)).toList();
        dataResponse.put("financialInfoRpList", financialInfoRpList);
        return DataResponseWrapper.builder().data(dataResponse).status("00000").message("")//todo: handler response message
                .build();
    }
    ...
    public DataResponseWrapper<Object> getDetailInfoById(String id, String transactionId) {
        ...
        return DataResponseWrapper.builder().data(financialInfoRp).status("00000").message("")//todo: handler response message
                .build();
    }
    ...
    public DataResponseWrapper<Object> approveFinancialInfo(ApproveFinancialInfoRq financialInfoRq, String transactionId) {
        Optional<FinancialInfo> financialInfoOptional = financialInfoRepository.findByIdAndIsDeleted(financialInfoRq.getFinancialInfoId(), false);
        if (financialInfoOptional.isEmpty()) {
        ...
        return DataResponseWrapper.builder().data(financialInfo.getId()).status("00000").message("")
                .build();
    }
    ...
    public DataResponseWrapper<Object> verifyFinancialInfo(String transactionId) {
        ...
        return DataResponseWrapper.builder().message("").data(isApprove).build();
    }
